...
### 9.3 核心开发流程（PR模式 + 任务管理 + TDD）

> **重要**：所有代码变更必须通过PR（Pull Request）方式合入main分支，禁止直接推送到main。
>
> **关键要求**：
> - **TDD强制要求**：先写测试，后写代码（RED → GREEN → REFACTOR）
> - 推送前必须完成本地构建验证，避免CI资源浪费
> - **任务必须先记录后执行**，所有开发任务必须写入 `docs/task/task_records.json`
> - **任务状态更新必须在提交远端之前完成**，提交远端后不再修改任务记录
> - **测试覆盖率要求**：ViewModel 80%+，Repository 80%+，工具类 90%+

```
┌─────────────────────────────────────────────────────────────────────┐
│          Harness核心开发流程（PR模式 + 任务管理 + TDD）               │
└─────────────────────────────────────────────────────────────────────┘
...
└─────────────────────────────────────────────────────────────────────┘
     ...
     └──────────────┬───────────────┘
                    ...
                    ▼
     ┌──────────────────────────────┐
     │  Step 15: 推送特性分支       │  ⚠️ 推送后不再修改
     │  git push -u origin feat/xxx │
     └──────────────┬───────────────┘
                    │
                    ▼
     ┌──────────────────────────────┐
     │  Step 16: 创建Pull Request   │
...
```json
{
  "task_id": "T20260324001",           // T + 日期(8位) + 序号(2位)
  "task_type": "功能开发/BUG修复/迭代优化/文档更新/测试编写",
  "task_desc": "任务描述（简洁明了）",
  "module": "ui/home",                  // 所属模块
  "executor": "Claude Code/人工",
  "status": "待执行/进行中/已完成/驳回",
  "create_time": "2026-03-24 09:00:00",
  "finish_time": "2026-03-24 12:00:00", // 未完成时为null
  "check_result": "通过/不通过",         // 未完成时为null
  "remark": "备注信息"
}
```
...
|---------|-----------|---------|
| 待执行 | 进行中 | 开始执行（Step 2） |
| 进行中 | 已完成 | 任务成功且通过校验（Step 6，提交前） |
| 进行中 | 驳回 | 任务执行失败或校验不通过 |

> **⚠️ 重要**：任务状态更新必须在提交远端之前完成（Step 6），推送远端后不再修改任务记录。
...
| 优先级 | 测试类型 | 覆盖率要求 | 示例 |
|--------|---------|-----------|------|
| P0 | 单元测试 | 80%+ | ViewModel、Repository、工具类 |
| P1 | 集成测试 | 70%+ | 数据库操作、API调用 |
| P2 | UI测试 | 50%+ | Activity、Fragment关键流程 |

**测试命名规范**：
```kotlin
// 格式：`when_条件_then_预期结果` 或 `given_条件_when_操作_then_结果`
@Test
fun `when load novels success then update state to Success`() { }

@Test
fun `given empty name when create novel then show error`() { }

@Test
fun `when delete novel then call repository delete`() { }
```

**测试目录结构**：
```
app/src/test/java/com/novelapp/aiagent/
├── viewmodel/           # ViewModel测试
│   ├── HomeViewModelTest.kt
│   └── EditorViewModelTest.kt
├── repository/          # Repository测试
│   └── NovelRepositoryTest.kt
├── ui/                  # UI组件测试（Robolectric）
│   ├── NovelListAdapterTest.kt
...
```bash
# ========== Step 1: 创建任务记录 ⚠️ 必须 ==========
# 编辑 docs/task/task_records.json，添加新任务
# 示例任务记录：
# {
#   "task_id": "T20260324025",
#   "task_type": "功能开发",
#   "task_desc": "实现HomeActivity小说列表功能",
#   "module": "ui/home",
...
# ========== Step 5: 运行测试，确认失败 🔴 ==========
./gradlew test --tests "com.novelapp.aiagent.viewmodel.HomeViewModelTest"
# 预期：测试失败（因为代码还没实现）

# ========== 🟢 GREEN Phase: Step 6-7 ==========
...
# 确保覆盖率达到80%+
# 查看报告：app/build/reports/coverage/test/debug/index.html

# ========== Step 13: 更新任务状态 ⚠️ 必须在提交前完成 ==========
# 编辑 docs/task/task_records.json，更新任务状态：
# {
#   "status": "已完成",
#   "finish_time": "2026-03-24 12:00:00",
#   "check_result": "通过",
#   "remark": "已完成小说列表展示、点击跳转、滑动删除功能，测试覆盖率85%"
# }

# ========== Step 14: 提交代码（包含代码+测试+任务记录）==========
git add .
...
## 测试用例
- HomeViewModelTest: 12个测试用例
- NovelListAdapterTest: 8个测试用例"
...
# ✅ 完成！任务记录已在Step 13更新，无需再次修改
```

#### 9.3.4 一键执行脚本（含TDD）

```bash
# 文件：scripts/create_pr_tdd.sh
...
TASK_FILE="docs/task/task_records.json"

echo "===== 开始PR流程（含任务管理 + TDD） ====="

# Step 1: 检查任务记录
echo "[1/18] 检查任务记录..."
if [ ! -f "$TASK_FILE" ]; then
    echo "⚠️  警告: $TASK_FILE 不存在，请先创建任务记录"
    exit 1
fi

# 提示确认任务已创建
echo "⚠️  请确认已在 $TASK_FILE 中创建任务记录且 status='待执行'"
read -p "任务已创建? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "请先创建任务记录后再执行"
    exit 1
...
# Step 2: 创建分支并更新任务状态
echo "[2/18] 创建特性分支: $BRANCH_NAME"
git checkout main && git pull origin main
git checkout -b "$BRANCH_NAME"
echo "⚠️  请更新 $TASK_FILE 中的任务状态为 '进行中'"

# Step 3: 分析需求
echo "[3/18] 分析需求，设计接口..."
echo "请确定：输入/输出、数据结构、边界条件"

# Step 4: 编写测试用例 🔴
echo "[4/18] 编写测试用例 🔴 RED Phase..."
echo "⚠️  TDD核心步骤：先写测试，再写代码"
read -p "测试用例已编写? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "请先编写测试用例"
    exit 1
...
# Step 5: 运行测试，确认失败 🔴
echo "[5/18] 运行测试，确认失败 🔴..."
./gradlew test || echo "✅ 测试失败符合预期（RED Phase）"

# Step 6: 编码实现 🟢
echo "[6/18] 编码实现 🟢 GREEN Phase..."
echo "⚠️  编写最小代码通过测试"
read -p "代码已实现? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "请先实现代码"
    exit 1
...
# Step 8: 重构优化 🔵
echo "[8/18] 重构优化 🔵 REFACTOR Phase..."
echo "可选：消除重复、提取公共逻辑、改善命名"
read -p "是否需要重构? (y/n) " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    read -p "重构完成? (y/n) " -n 1 -r
    echo
    # Step 9: 再次运行测试
    echo "[9/18] 运行测试，确认重构未破坏功能 🔵..."
    ./gradlew test
else
    echo "[9/18] 跳过重构步骤"
fi
...
# Step 11: 本地构建验证
echo "[11/18] 本地构建验证（预计5-10分钟）..."
./gradlew assembleDebug

# Step 12: 验证测试覆盖率 ⚠️ 必须
echo "[12/18] 验证测试覆盖率..."
./gradlew testDebugUnitTestCoverage || echo "⚠️  请检查覆盖率报告"
echo "覆盖率报告: app/build/reports/coverage/test/debug/index.html"

# Step 13: 更新任务状态 ⚠️ 必须在提交前完成
echo "[13/18] 更新任务状态..."
echo "⚠️  请更新 $TASK_FILE 中的任务状态为 '已完成'"
echo "   - status: '已完成'"
echo "   - finish_time: 当前时间"
echo "   - check_result: '通过'"
echo "   - remark: 包含测试覆盖率信息"
read -p "已更新任务状态? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "请先更新任务状态后再提交"
    exit 1
...
# Step 15: 推送分支 ⚠️ 推送后不再修改
echo "[15/18] 推送分支..."
echo "⚠️  推送后不再修改任务记录"
git push -u origin "$BRANCH_NAME"

# Step 16: 创建PR
echo "[16/18] 创建Pull Request..."
...
# Step 17: 等待门禁
echo "[17/18] 等待门禁检查和合并..."
echo "查看门禁状态: gh pr checks"

# Step 18: 合并提示
echo "[18/18] 等待合并..."
echo "门禁通过后执行: gh pr merge --squash --delete-branch"

echo "===== PR创建成功！ ====="
echo "✅ TDD流程完成，任务记录已在提交前更新"
```
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.7.0 | 2026-03-25 | 核心流程整合TDD：新增RED-GREEN-REFACTOR三阶段（Step 3-9），强制测试优先，覆盖率要求80%+ |
| v1.6.1 | 2026-03-24 | 优化任务管理流程：任务状态更新调整到提交远端之前（Step 6），推送后不再修改 |
| v1.6.0 | 2026-03-24 | 核心开发流程整合任务管理，新增Step 1/11任务记录步骤，强制任务先记录后执行 |
